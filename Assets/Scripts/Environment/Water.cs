		...
		public void Update()
        {
            if (playerSwimming)
            {
                if (!lowWater)
                {
                    ...
                    if (!playerIsElephant)
                        player.transform.position = new Vector3(player.transform.position.x, playerPositionY - (GetComponent<Collider>().bounds.extents.y / 2) + .5f, player.transform.position.z);
                }
                else
                    swimConstraints = RigidbodyConstraints.FreezeRotation;
            }
